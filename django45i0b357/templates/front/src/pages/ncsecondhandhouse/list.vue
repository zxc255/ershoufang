		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				<el-form-item class="list-item">
					...
					<el-input v-model="formSearch.region" placeholder="区域" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<el-button class="list-search-btn" v-if=" true " type="primary" @click="getList(1, curFenlei)">
					<i class="el-icon-search"></i>
					查询
				</el-button>
				<el-button class="list-add-btn" v-if="btnAuth('ncsecondhandhouse','新增')" type="primary" @click="add('/index/ncsecondhandhouseAdd')">
			...
			<div class="list">
				<div class="list6">
					<div v-for="(item,index) in dataList" :key="index" :class="'list-item' + ((index%2) + 1)" @click.stop="toDetail(item)">
						<div class="imgbox">
							<img @click.stop="imgPreView(item.cover.split(',')[0])" v-if="item.cover && item.cover.substr(0,4)=='http'" :src="item.cover.split(',')[0]" class="image" />
							<img @click.stop="imgPreView(baseUrl + (item.cover?item.cover.split(',')[0]:''))" v-else :src="baseUrl + (item.cover?item.cover.split(',')[0]:'')" class="image" />
						</div>
						<div class="infoBox">
							<div class="name">{{item.region}}</div>
	...
	export default {
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				let searchWhere = {};
				if (this.formSearch.region != '') searchWhere.region = '%' + this.formSearch.region + '%';
				let user = JSON.parse(localStorage.getItem('sessionForm'))
				if (this.sortType) searchWhere.sort = this.sortType
				if (this.sortOrder) searchWhere.order = this.sortOrder
				this.$http.get(`ncsecondhandhouse/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/ncsecondhandhouseDetail', query: params});
			},
