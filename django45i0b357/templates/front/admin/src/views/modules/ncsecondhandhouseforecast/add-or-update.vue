<template>
	<div class="addEdit-block">
		...
		>
			<template >
				<el-form-item class="input" v-if="type!='info'"  label="总价" prop="totalprice" >
					<el-input-number v-model="ruleForm.totalprice" placeholder="总价" :disabled="ro.totalprice"></el-input-number>
				</el-form-item>
				<el-form-item v-else class="input" label="总价" prop="totalprice" >
...
<script>
	...
	export default {
		...
		methods: {
			...
			info(id) {
				this.$http({
					url: `ncsecondhandhouseforecast/info/${id}`,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
...
var objcross = this.$storage.getObj('crossObj');
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `ncsecondhandhouseforecast/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.ncsecondhandhouseforecastCrossAddOrUpdateFlag = false;
											this.parent.search();
